...
%but should consider at some point

%filter for eeg


%make ripple filtered eeg traces

for chCtr = 1:length(layerChans)
    chCtr = chCtr+1;
    %ripple filter (150-250 Hz)
    ripple.filtdata(chCtr,:) = filtfilt(ripplefilter.kernel, 1 , tmpDat(chCtr,:));
