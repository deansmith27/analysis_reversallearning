    ...
    classdef ChoiceAnalyzer < handle 
        ...
        methods
            ...
            function get_dynamic_choice(obj)
                 ...
                 learning_rate = obj.params.learning_rate;    %extract values of grid_search_params

                    for iBatch = 1:numel(batch_size) % nested for loops to make Cartesian products
                        batch_size = batch_size(iBatch); %assigns column to variable
                        for iEpochs =  1:numel(epochs)
                            epochs = epochs(iEpochs);
                            for iReg =  1:numel(regularizer)
                                regularizer = regularizer(iReg);
                                for iLR =  1:numel(learning_rate)
                                    learning_rate = learning_rate(iLR);
