...
for id = 1:length(params.rocID)
    ...
    for ss = 1:size(uniqSess,1)
        ...
        % Danielle: Code to stop it from breaking if sessionInfo is empty
        ...
        % Danielle debug code:
        % disp(['making ROC', num2str(animal), ' ', num2str(sessionInfo(1,2)), ' ', params.rocID{id}])
...
%% 
        ...
        %% Deandra's ROC type 1: use all trials and anticipatory zones vs primary control zone
        % This is attempt two with added with want to create new variable
...
%%  
        ...
        % 
        %             %reshape to Nx1 structure and combine AZ and CZ data into one
        %             % Deandra: reshapes because of the 2 and now there is an average across columns (one value per row)
        %             groupData_az = nanmean(groupData.(currEnv)(g).az, 2); % takes the averages across columns, ignoring NaN values while extracting on az values of the currEnv in groupData
        %             groupData_cz = nanmean(groupData.(currEnv)(g).cz, 2);
        %             roc_groupData = calcBehaviorROC(groupData_az*params.rocMultiplier(id), groupData_cz*params.rocMultiplier(id));
        % 
        % 
        ...
        % 
        %         else
        %             rocOut.(currEnv)(g).AUC(ss).trialblock = nan;
        %         end
        %     end
        % end
        %% 
