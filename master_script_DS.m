...
addpath('\\ad.gatech.edu\bme\labs\singer\01_PEOPLE\Danielle\code\vr_novelty_behavior-xiao\commonfunc')
...
if createBehaviorStructs
    ...
    for i = 1:size(allindex,1) %loop through every session
        addpath('\\ad.gatech.edu\bme\labs\singer\01_PEOPLE\Danielle\code\vr_novelty_behavior\commonfunc')
            ...
            if isfolder(virmenSessDataPath)%ensure behavior structs created using code above
                ...
                if postSpikeSort
                    ...
                    if ~isfile([processedDataPath '\kilosort4\' sprintf('%s_%s.cell_metrics.cellinfo.mat', subj, sessDate)]) || params.rewrite.cell_metrics
                        ...
                        addpath(genpath('\\ad.gatech.edu\bme\labs\singer\01_PEOPLE\Danielle\code\CellExplorer'));
